...
const accountDataSchema = z.object({
    ...
    pseudo: z.string().min(3, { message: error.minCharPseudo }).max(30, { message: error.maxChar }).optional(),
    picture: z.string().min(10, { message: error.invalidPicture }).max(1000, { message: error.maxChar }).optional(),
});
...
export const profilSchema = accountDataSchema.omit({ id: true, isAdmin: true });
